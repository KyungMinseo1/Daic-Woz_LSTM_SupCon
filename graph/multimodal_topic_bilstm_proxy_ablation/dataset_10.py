...
def make_graph(ids, labels, model_name, colab_path=None, use_summary_node=True, t_t_connect=False, v_a_connect=False, visualization=False, explanation=False):
  try:
    ...
    for graph_idx, id in tqdm(enumerate(filtered_ids), desc="Dataframe -> Graph", total=len(filtered_ids)):
      if colab_path is not None:
        df = pd.read_csv(os.path.join(colab_path, 'Annotated', f"annotated_{id}_transcript.csv"))
        ...
        df = pd.read_csv(os.path.join(path_config.DATA_DIR, 'Annotated', f"annotated_{id}_transcript.csv"))
      ...
      try:
        ...
        df['topic'] = df['topic_annotation'].ffill()
        df = df.reset_index()
        ...
        for t_emb, topic, (start, stop) in zip(t_embeds, topics, start_stop_list):
          ...
          if v_a_connect and (v_node_id is not None and a_node_id is not None):
            if v_node_id == a_node_id - 1:
              source_nodes.append(v_node_id)
        ...
        if len(audio_seq_list) > 0:
            ...
            if x_audio.shape[-1] != AUDIO_FEAT_DIM:
                if x_audio.shape[-1] > AUDIO_FEAT_DIM:
                    x_audio = x_audio[:, :, :AUDIO_FEAT_DIM] # 넘치면 자르기
                else:
                    pad = torch.zeros((x_audio.shape[0], x_audio.shape[1], AUDIO_FEAT_DIM - x_audio.shape[-1]))
                    x_audio = torch.cat([x_audio, pad], dim=-1) # 부족하면 채우기
            
            data_audio_lengths = torch.tensor(audio_lengths_list, dtype=torch.long)
        else:
            x_audio = torch.empty((0, MAX_SEQ_LEN_AUDIO, AUDIO_FEAT_DIM))

...
from .dataset_10 import make_graph
...
def main():
  ...
  optimizer = torch.optim.AdamW(
    model.parameters(), 
    lr=0.000029,          # Learning Rate
    weight_decay=0.000965 # Weight Decay
  )
  ...
  for epoch in range(starting_epoch, starting_epoch + opt.num_epochs + 1):
    ...
    history_path = os.path.join(CHECKPOINTS_DIR, 'training10_history.csv')
    history_df.to_csv(history_path, index=False)
    # logger.info(f"Training history updated at {history_path}") # 매번 출력하면 지저분하니 생략 가능
